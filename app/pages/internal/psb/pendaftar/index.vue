  ...
  <div class="pb-20">
    ...
    <div class="space-y-3">
      ...
      <div v-for="item in filteredPendaftar" :key="item.id_pendaftar" 
        ...
        <div class="hidden md:flex md:col-span-1 justify-end gap-2"> 
          <button @click="$router.push(`/internal/psb/pendaftar/edit/${item.id_pendaftar}`)"
                  class="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-blue-400 hover:bg-blue-600 hover:text-white transition-all border border-white/10">
            <span class="text-xs">✏️</span>
          </button>
